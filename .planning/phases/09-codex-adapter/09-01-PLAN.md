...
Output: ApprovalPolicy enum, updated build_args(), module-level documentation, flag combination tests, clippy pedantic clean.
</objective>
...
Note: The field defaults to `None` (no flag generated), but `ApprovalPolicy::default()` is `Untrusted`. This allows:
- `None`: CLI uses its internal default (no flag passed)
- `Some(ApprovalPolicy::Untrusted)`: Explicitly sets most restrictive mode (recommended for containment)

The locked decision "Default approval policy: untrusted" is honored by making `Untrusted` the `#[default]` variant, so `ApprovalPolicy::default()` returns `Untrusted`.
  </action>
  <verify>cargo check -p codex-adapter (compiles without errors)</verify>
  <done>ApprovalPolicy enum exists with 4 variants, Untrusted is #[default], ask_for_approval field added to CodexConfig with documentation explaining containment-first usage</done>
</task>
...
```
  ...
  <verify>cargo doc -p codex-adapter --no-deps (docs render without warnings)</verify>
...
<task type="auto">
  <name>Task 3: Add flag combination unit tests and run clippy pedantic pass</name>
  <files>codex-adapter/src/cmd.rs</files>
  <action>
**Part A: Add unit tests**
...
**Part B: Run clippy pedantic pass and fix all warnings**

Run `cargo clippy -p codex-adapter -- -W clippy::pedantic` and resolve ALL warnings.
...
- Use `Option::map_or` instead of `match` where cleaner

This is a LOCKED DECISION from CONTEXT.md: "Codex-specific clippy pedantic pass (not relying on workspace-wide 08-01)".

After fixing, verify with: `cargo clippy -p codex-adapter -- -W clippy::pedantic 2>&1 | grep -c warning` returns 0.
  </action>
  <verify>cargo clippy -p codex-adapter -- -W clippy::pedantic (zero warnings) && cargo test -p codex-adapter (all tests pass)</verify>
  <done>5+ new flag combination tests using windows(2) pattern, all tests pass, clippy pedantic returns zero warnings</done>
</task>

</tasks>

<verification>
1. `cargo clippy -p codex-adapter -- -W clippy::pedantic` - zero warnings
...
5. Verify ApprovalPolicy::default() returns Untrusted (containment-first default)
</verification>

<success_criteria>
- ApprovalPolicy enum with Untrusted, OnFailure, OnRequest, Never variants
- Untrusted is #[default] variant (locked decision: containment-first)
- ask_for_approval field on CodexConfig with containment documentation
- build_args() generates --ask-for-approval flag correctly
...
- All clippy pedantic warnings resolved (locked decision: Codex-specific pass)
</success_criteria>
