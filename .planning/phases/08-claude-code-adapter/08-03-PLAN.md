...
must_haves:
  truths:
    - "E2E tests verify containment with real Claude CLI"
    - "Tests are marked #[ignore] for CI compatibility"
    - "Tests validate flags actually restrict behavior (not just generated)"
    - "Requirements documented in test module docstring"
  artifacts:
    - path: "claudecode-adapter/tests/e2e_containment.rs"
      provides: "E2E containment tests requiring real CLI"
      contains: "#[ignore = \"Requires Claude CLI installed\"]"
  key_links:
    - from: "claudecode-adapter/tests/e2e_containment.rs"
      to: "claude CLI binary"
      via: "subprocess execution"
      pattern: "ClaudeCli::new"
---

<objective>
Add E2E tests that validate containment features with the real Claude CLI.

Purpose: Unit tests verify flag generation, but only E2E tests verify flags actually work. Containment is critical - agent must be restricted to MCP tools only when configured.

Output: E2E test file with #[ignore] tests that validate real containment behavior.
</objective>
...
<task type="auto">
  <name>Task 1: Add dev-dependencies for E2E testing</name>
  <files>claudecode-adapter/Cargo.toml</files>
  <action>
Add dev-dependencies for E2E testing to claudecode-adapter/Cargo.toml:
...
The adapter already has tokio as a dependency, but we need the test-util feature for E2E tests.
tempfile is needed for creating isolated test directories.
  </action>
  <verify>
Run: `cargo check -p claudecode-adapter --tests` compiles successfully
  </verify>
  <done>Dev-dependencies added for E2E testing</done>
</task>

<task type="auto">
  <name>Task 2: Create E2E containment test file</name>
  <files>claudecode-adapter/tests/e2e_containment.rs</files>
  <action>
Create `claudecode-adapter/tests/e2e_containment.rs` with E2E containment tests:
...
use claudecode_adapter::{
    init, run_claude, BuiltinToolSet, ClaudeCli, McpPolicy, RunConfig, ToolPolicy,
};
...
```
  ...
  <verify>
Run: `cargo test -p claudecode-adapter --test e2e_containment` compiles without errors
Note: Tests won't run (ignored) but should compile successfully
  </verify>
  <done>E2E containment test file created with 4 #[ignore] tests</done>
</task>
...
```bash
cargo test -p claudecode-adapter --test e2e_containment --no-run
```
...
Expected: Tests should compile. When run with `--ignored`:
- e2e_containment_no_builtins: Verifies builtin tools are disabled
- e2e_containment_allowed_tools_only: Verifies tool restriction
- e2e_disable_slash_commands: Verifies flag is accepted
- e2e_timeout_graceful_shutdown: Verifies timeout path
</verification>

<success_criteria>
- [ ] tests/e2e_containment.rs created with 4 E2E tests
- [ ] All tests marked `#[ignore = "Requires Claude CLI installed"]`
- [ ] Module-level documentation explains requirements and running instructions
- [ ] Dev-dependencies (tempfile) added to Cargo.toml
- [ ] Tests compile with `cargo test --test e2e_containment --no-run`
- [ ] Tests are structured to validate behavior, not just flag generation
</success_criteria>

<output>
After completion, create `.planning/phases/08-claude-code-adapter/08-03-SUMMARY.md`
</output>
