...
must_haves:
  ...
  artifacts:
    - path: "opencode-adapter/src/cmd.rs"
      provides: "CLI flag documentation and containment explanation"
...
<objective>
Add comprehensive module-level documentation to OpenCode adapter matching Claude Code and Codex adapter quality.

Purpose: Production parity requires same documentation depth. OpenCode has different containment mechanisms (working directory + env vars instead of CLI flags), which must be clearly documented.
Output: Module-level docs in cmd.rs and lib.rs, clippy pedantic clean.
</objective>
...
Keep all existing code unchanged. The tests already document containment behavior inline.
  </action>
  <verify>cargo doc -p opencode-adapter --no-deps (docs render without warnings)</verify>
  <done>cmd.rs has module-level documentation with Flag Reference, Containment Strategy, Containment Comparison table, Version Notes, Known Limitations, and External References sections</done>
</task>
...
<task type="auto">
  <name>Task 3: Run clippy pedantic pass and fix all warnings</name>
...
2. `cargo test -p opencode-adapter` - all existing tests pass
